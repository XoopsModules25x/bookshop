...
$xoopsOption['template_main'] = 'bookshop_command.html';
...
function listCart()
{
	...
	$shippingAmount = sprintf("%0.".$dec.'f', $shippingAmount);		// Montant des frais de port
	$commandAmount =  sprintf("%0.".$dec.'f', $commandAmount);		// Montant HT de la commande
	$vatAmount = sprintf("%0.".$dec.'f', $vatAmount);				// Montant de la TVA
	$commandAmountTTC = sprintf("%0.".$dec.'f', $commandAmountTTC);	// Montant TTC de la commande (frais de port inclus)
}
...
switch ($op)
{
	...
	// ****************************************************************************************************************
		...
		foreach($cartForTemplate as $line) {
			...
			$msgCommande .= str_pad(wordwrap($line['book_title'], 60), 60, ' ').' '.str_pad($line['book_qty'].' '._BOOKSHOP_COPY_COUNT,8, ' ', STR_PAD_LEFT).' '.str_pad($line['book_price_ttc'].' '.bookshop_getmoduleoption('money_short'),10,' ',STR_PAD_LEFT).' '.str_pad($line['book_shipping_amount'].' '.bookshop_getmoduleoption('money_short'),10,' ',STR_PAD_LEFT)."\n";
