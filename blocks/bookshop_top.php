...
//  ------------------------------------------------------------------------ //
/**
 * Affiche les livres les plus vus
 */
function b_bookshop_top_show($options)
{
	...
	$tblLivres = $h_bookshop_books->getMostViewedBooks(0, $options[0], $options[1]);
 ...
 * Paramètres du bloc
 */
function b_bookshop_top_edit($options)
{
 ...
 * Bloc à la volée
 */
function b_bookshop_top_show_duplicatable($options)
{
	$options = explode('|',$options);
	$block = & b_bookshop_top_show($options);

	$tpl = new XoopsTpl();
	$tpl->assign('block', $block);
	$tpl->display('db:bookshop_block_top.html');
}
?>