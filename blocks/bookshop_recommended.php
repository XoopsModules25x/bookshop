 ...
 * Created on 22 déc. 07 at 18:12:51
 * ****************************************************************************
 */
...
function b_bookshop_recomm_show($options)
{
	...
	if($h_bookshop_books->getRecommendedCount() > $options[0]) {	// Il y a plus de livres recommandés dans la BDD que dans le bloc, on affiche donc un lien vers la page des livres recommandés
		$block['showMore'] = true;
	}
...
function b_bookshop_recomm_duplicatable($options)
{
	...
	$tpl->display('db:bookshop_block_recommended.html');
}
?>