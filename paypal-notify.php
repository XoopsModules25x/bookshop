...
//  ------------------------------------------------------------------------ //
/**
 * Page called by Paypal in the case of the use of the IPN
 */
@error_reporting(0);
...
if ($fp) {
    ...
    while (!feof($fp)) {
        ...
        if (strcmp($res, 'VERIFIED') == 0) {
            ...
            if (strtoupper($_POST['receiver_email']) != strtoupper(bookshop_getmoduleoption('paypal_email'))) {
                $paypalok = false;
            }
            if (strtoupper($_POST['mc_currency']) != strtoupper(bookshop_getmoduleoption('paypal_money'))) {
                $paypalok = false;
            }
            if (!$_POST['custom']) {
                $paypalok = false;
            ...
            if ($paypalok) {
                ...
                if (is_object($commande)) {
                    ...
                    if ($montant == $commande->getVar('cmd_total')) {    // checked Command
                        $h_bookshop_commands->validateCommand($ref);    // Order validation and updating of stocks
